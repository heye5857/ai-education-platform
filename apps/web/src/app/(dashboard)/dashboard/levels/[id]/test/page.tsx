...
export default function LevelTestPage() {
  ...
  const paperQuery = useQuery({
    ...
    queryFn: () =>
      api.get<TestPaper>(`/api/learning/level-test?levelId=${encodeURIComponent(levelId)}`),
    retry: false,
  })
  ...
  const submitMutation = useMutation({
    ...
    }) => api.post<TestResult>('/api/learning/level-test/submit', payload),
        ...
        <Card className={cn(result.passed ? 'border-success-500/40' : 'border-warning-500/40')}>
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">
              {result.passed ? '通過！🎉' : '再接再厲💪'}
            </CardTitle>
            <p className="text-muted-foreground">
              得分 {result.score} / {result.maxScore}（通過標準 {result.passScore} 分）
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <Button variant="ghost" size="sm" onClick={() => router.push(`/dashboard/levels/${levelId}`)}>
          <ArrowLeft className="h-4 w-4 mr-1" aria-hidden="true" />
          回關卡
        </Button>
